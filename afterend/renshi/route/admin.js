...
// 将路由对象作为模块成员进行导出
admin.post('/Text-add',require('./admin/Text-add'));
// 将路由对象作为模块成员进行导出


admin.get('/Text-edit',async(req,res)=>{
    const {message,id} = req.query;
    if(id){
       let audioText =await AudioText.findOne({_id:id});
       res.render('admin/Text-edit',{
           message:message,
           audioText:audioText,
           link:'/admin/Text-modify?id='+id,
           button:"修改"
        });
    }else{
        res.render('admin/Text-edit',{
...
admin.get('/delete-Text',async(req,res)=>{
    await AudioText.findOneAndDelete({_id:req.query.id});
    res.redirect('/admin/Text');      
});
admin.get('/Text',async(req,res)=>{
    req.app.locals.currentLink = 'Text';
    let  audioText = await pagination(AudioText).find().page(1).size(10).display(3).exec();
    res.render('admin/Text',{
        audioText:audioText
    });
});
admin.post('/Text-modify',async(req,res)=>{
        const id = req.query.id;
           await Article.updateOne({_id:id},{
            title:req.body.title,
            author:req.body.author,
            prenenter:req.body.prenenter,
...
const clients = new AsrClient(clientConfig);

admin.get('/Text-show',async(req,res)=>{
    const id = req.query.id;
    let audioText =await AudioText.findOne({_id:id});
    const params = {
        "TaskId": audioText.TaskId
    };
    console.log(params);
    clients.DescribeTaskStatus(params).then(
        (data) => {
        console.log(data);
        },
        (err) => {
...
admin.post('/yanjiangarticle-add',require('./admin/yanjiangarticle-add'));
admin.post('/getArticle',require('./admin/getArticle'))
admin.post('/gettupu',require('./admin/gettupu'))
...
admin.post('/getdg',require('./admin/getdg'))
